...
func _process(delta: float) -> void:
	...
	if biome and not biome.weather_enabled:
		current_blend = move_toward(current_blend, 0.0, transition_speed * delta)
	else:
		var target_blend: float = 0.0
		
		# Check if ranges are configured
		if biome and moisture_max > moisture_min and barometer_max > barometer_min:
			var current_moisture = biome.biome_environment.get_moisture()
			var current_barometer = biome.biome_environment.get_barometer()
			var current_temperature = biome.biome_environment.get_temperature()
		...
		current_blend = move_toward(current_blend, target_blend, transition_speed * delta)
	...
	var target_color = gradient.gradient.sample(time_value)
	color = color.lerp(target_color, clamp(transition_speed * delta, 0.0, 1.0))
